...
// Generate new subtraction problem with 3 or 4 digit numbers
function generateNewProblem() {
  ...
  // Generate second number first
  secondNumber.value = generateRandomNumber(digitCount2);

  // Make sure first number is larger than second number
  const minFirstNumber = secondNumber.value + 1;
  const maxFirstNumber = Math.pow(10, digitCount1) - 1;

  // Ensure first number has the required digits
  if (minFirstNumber > maxFirstNumber) {
    // If we need more digits for the first number
    firstNumber.value = generateRandomNumber(digitCount1 + 1);
  } else {
    firstNumber.value =
